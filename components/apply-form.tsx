...
export function ApplyForm({ job, profile }: ApplyFormProps) {
  ...
  // Phone state with validation for WhatsApp notifications
  const [phone, setPhone] = useState(profile.phone || "");
  const [phoneTouched, setPhoneTouched] = useState(!!profile.phone);
    ...
    if (!phoneValidation.isValid) {
      setErrorMessage(
        phoneValidation.error ||
          "Format nomor WhatsApp tidak valid. Masukkan nomor WhatsApp yang benar (contoh: 0812-3456-7890 atau +6281234567890)."
      );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card-3d rounded-3xl p-8 bg-white border border-slate-200/80 shadow-soft-3d space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* WhatsApp Phone Number for Instant Selection Notifications */}
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-semibold text-slate-700">
                Nomor WhatsApp Notifikasi Seleksi <span className="text-rose-500">*</span>
              </label>
              <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
            ...
            <div className="relative">
              <Input
                ...
                className={`pl-9 pr-9 rounded-xl transition-colors ${
            ...
            {phoneTouched && phone.trim().length > 0 ? (
              isPhoneValid ? (
                <p className="text-[11px] text-emerald-600 font-medium mt-1 flex items-center gap-1">
                  ...
                  <span>Format nomor valid: {phoneValidation.localDisplay} (WhatsApp Terhubung)</span>
                </p>
              ) : (
              ...
              <p className="text-[11px] text-slate-500 mt-1">
                Wajib nomor WhatsApp terdaftar. Update status screening AI dan undangan interview akan dikirimkan ke nomor ini.
              </p>
            )}
          </div>
